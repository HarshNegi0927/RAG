...
Every LLM call uses **forced tool-use** (`tool_choice`), not "please respond in JSON" — the Pydantic models in `src/schemas.py` are the actual `input_schema`, so there's one source of truth for the data shape instead of a schema that quietly drifts from a regex parser somewhere.
...
**Model:** defaults to `openai/gpt-oss-120b` on Groq -- a current production model with native tool-calling, on the free tier. Set the `model=` argument in any of `extract_profile()` / `match_profiles()` / `generate_report()` (or `GROQ_MODEL` in `demo.html`) to swap models; check [console.groq.com/docs/models](https://console.groq.com/docs/models) for what's currently active, since Groq deprecates/rotates free-tier models periodically.
...
src/extraction.py             unstructured text -> structured profile (LLM, forced tool-use)
src/matcher.py                requirement-by-requirement fit reasoning (LLM, forced tool-use)
